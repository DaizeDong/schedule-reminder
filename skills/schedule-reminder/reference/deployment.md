...
| Thing | Where | Notes |
|---|---|---|
| DB (3 files) | `~/.claude/schedule-reminder/db.sqlite3` (+`-wal`, `-shm`) | **local NTFS only** — never OneDrive/GDrive/network (WAL file-lock + sync corruption) |
...
Task XML knobs (Beeline-style): `StartWhenAvailable=true`, `UseUnifiedSchedulingEngine=true`,
...
- `SCHEDULE_RELAY_SEND` — path to `discord_relay/send.py` (default `~/.claude/discord_relay/send.py`).
...
```bash
sqlite3 ~/.claude/schedule-reminder/db.sqlite3 "VACUUM INTO 'backup.sqlite3'"
```
