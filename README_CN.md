...
[Windows 任务: PT5M 心跳] -> reminder.py tick -> 对账到期项 -> Discord relay
```
...
13 个评测信号（E1-E13）全程经 subprocess 调冻结 CLI、断言 JSON：CRUD、完整状态转移表（合法 + 非法）、写入
不变量、到期触发 / 幂等 tick / 错过补发 / 重试退避、并发写 + `PRAGMA integrity_check`、并发读写、幂等去重、
API golden、未知字段保留、health。E8/E9/E11/E12 为阻断合并的红线。
